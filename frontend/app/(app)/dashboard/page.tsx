...
export default function Dashboard() {
  ...
  const STAT_CARDS = [
    { label: "Total Papers",   value: stats.total,     color: "#94A3B8" },
    ...
    { label: "In Review",      value: stats.submitted, color: "#3B82F6" },
    { label: "Disputes",       value: stats.disputes,  color: "#94A3B8" },
  ];
  ...
  return (
    <div className="space-y-8">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-2xl font-bold text-[#F5F7FA]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Dashboard</h1>
          <p className="text-sm text-[#94A3B8] mt-1">Research corpus overview</p>
        </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold text-[#F5F7FA]" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Your Corpora</h2>
          <Link href="/corpus/create" className="text-sm text-[#3B82F6] hover:opacity-80">+ New</Link>
        </div>
        {corpora.length === 0 ? (
          <div className="card text-center py-12">
            <p className="text-[#94A3B8] mb-4">No corpora yet.</p>
            <Link href="/corpus/create" className="btn-primary">Create your first corpus</Link>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
            {corpora.map((c) => (
              <Link key={c.corpus_id} href={`/corpus/${c.corpus_id}`} className="card hover:border-[#3B82F6]/40 transition-colors block">
                <div className="flex items-start justify-between mb-3">
                  <span className="text-xs font-mono text-[#94A3B8] truncate max-w-[140px]">{c.corpus_id}</span>
                  <span className={`badge ${c.status === "ACTIVE" ? "badge-accept" : "badge-submitted"}`}>{c.status}</span>
                </div>
                <h3 className="font-semibold text-[#F5F7FA] mb-2" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>{c.name}</h3>
                <p className="text-sm text-[#94A3B8] line-clamp-2">{c.description}</p>
                <div className="mt-3 flex gap-2 flex-wrap">
                  {c.include_topics?.slice(0, 3).map((t) => (
                    <span key={t} className="text-xs px-2 py-0.5 rounded bg-[#1E2D42] text-[#94A3B8] font-mono">{t}</span>
                  ))}
                </div>
                <div className="mt-3 pt-3 border-t border-[#1E2D42] flex gap-4 text-xs text-[#94A3B8]">
      ...
      {recentPapers.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-[#F5F7FA] mb-4" style={{ fontFamily: "'Space Grotesk', sans-serif" }}>Recent Papers</h2>
          <div className="card p-0 overflow-hidden">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#1E2D42]">
                  <th className="text-left p-4 text-xs text-[#94A3B8] font-normal">Paper</th>
                  <th className="text-left p-4 text-xs text-[#94A3B8] font-normal hidden md:table-cell">Corpus</th>
              ...
              <tbody>
                {recentPapers.map((p) => (
                  <tr key={p.paper_id} className="border-b border-[#1E2D42]/50 hover:bg-[#1E2D42]/20">
                    <td className="p-4 text-[#F5F7FA] font-medium max-w-xs truncate">{p.title}</td>
                    <td className="p-4 text-[#94A3B8] font-mono text-xs hidden md:table-cell">{p.corpus_id}</td>
